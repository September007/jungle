...
BOOST_DEFINE_ENUM(Data_Type, Invalid, Integer, Floating, String,
                  Table_ /*, Compose*/);
...
using InvalidType = struct InvalidType {
  constexpr bool operator<(InvalidType const &) { return true; }
};
...
inline std::shared_ptr<Table> Construct_Table(Args... ags);

using _DataItem_Variant =
    std::variant<InvalidType, Int, Float, std::string, std::shared_ptr<Table>>;
struct DataItem : public _DataItem_Variant {
  // using std::variant<InvalidType, Int, Float, std::string,
  //                    std::shared_ptr<Table>>::variant;
  bool operator<(DataItem const &ot) const {
    if (type != ot.type)
      return type < ot.type;
    switch (type) {
    ...
    case Data_Type::Table_:
      return Table_Less(std::get<std::shared_ptr<Table>>(*this),
                        std::get<std::shared_ptr<Table>>(ot));
...
struct Table : public std::map<DataItem, DataItem> {
  ...
  friend bool Table_Less(std::shared_ptr<Table> l, std::shared_ptr<Table> r) {
...
struct DataBase : public VariantDataBase {
  ...
  // cpp-end data access
#define ist(T)                                                                 \
  bool is##T() { return type == Data_Type::T; }
  ist(Invalid);
  ist(Integer);
  ist(Floating);
  ist(String);
